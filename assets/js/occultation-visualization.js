...
// 文件更新监测相关变量（通过 manifest.updated_at 判断）
let lastManifestUpdatedAt = '';
...
async function checkFileUpdates() {
    try {
        const response = await fetch(manifestFile, { cache: 'no-store' });
        if (!response.ok) {
            return;
        }
        ...
        if (lastManifestUpdatedAt && updatedAt !== lastManifestUpdatedAt) {
            console.log('检测到 manifest 更新，重新加载数据...');
            lastManifestUpdatedAt = updatedAt;
            await reloadData(currentViewer);
            return;
        }
        if (!lastManifestUpdatedAt) {
            lastManifestUpdatedAt = updatedAt;
        }
    } catch (error) {
...
function startFileMonitoring(viewer) {
    ...
    fetch(manifestFile, { cache: 'no-store' })
        .then(response => response.ok ? response.json() : null)
        .then(manifest => {
            if (manifest && manifest.updated_at) {
                lastManifestUpdatedAt = manifest.updated_at;
            }
        })
        .catch(error => console.error('获取 manifest 失败:', error));
